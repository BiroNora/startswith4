...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const eventCounts = schoolsWithEventCounts.map((school) => {
		...
		estimatedStudent = school.Event.reduce(
			(total, event) => total + (event.estimated_student || 0),
			0
		)
	...
	const interestedWithStat0: InterestedStudent[] = allInterestedStudents.filter(
		(student) => student.status === '0'
	)

	const interestedWithStat1: InterestedStudent[] = allInterestedStudents.filter(
		(student) => student.status === '1'
	)

	const interestedWithStat2: InterestedStudent[] = allInterestedStudents.filter(
		(student) => student.status === '2'
	)

	const interestedWithStat3: InterestedStudent[] = allInterestedStudents.filter(
	...
	const totalIntrest0: number = interestedWithStat0.reduce(
		(total, student) => total + student.intrest_count,
		0
	)
